...
Проверка организации остаётся на отдельной странице проверки карты. Во время
загрузки XLSX и ручной отправки возвратов она временно отключена: загрузка
сохраняет обращения без ожидания Datareon и передаёт пустой `organization_id`.
БИК проверяется как 9 цифр; пробелы внутри телефона и расчётного счёта удаляются.
